...
function extractJsonObject(text) {
  ...
  throw new Error(`Failed to parse scene JSON. Preview: ${excerpt}`);
}
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt += 1) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const upstreamError = buildUpstreamError(message, isRetryableStatus(response.status));
        upstreamError.status = response.status;
        upstreamError.response = parsed;
...
function buildResponsesPayload(meta, prompt, options) {
  return {
    model: options.model,
    store: false,
    service_tier: options.serviceTier || undefined,
    reasoning: options.reasoningEffort ? { effort: options.reasoningEffort } : undefined,
    instructions: "You are a precise multimodal parser for physics circuit diagrams. Return valid JSON only and do not add commentary.",
    text: {
...
async function requestSceneJson(meta, prompt, options) {
  const response = await callResponsesApi(buildResponsesPayload(meta, prompt, options));
