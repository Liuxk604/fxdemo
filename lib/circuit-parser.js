...
function getEnvConfig() {
  return {
    ...
    wireApi: process.env.OPENAI_WIRE_API || DEFAULT_WIRE_API,
...
function normalizeWireApi(value) {
  ...
  if (["chat_completions", "chat_completion", "chatcompletions"].includes(normalized)) {
...
function buildPrompt(meta) {
      ...
      measurements: [
        {
          ...
          description: "Optional measurement derived from visible or known quantities"
        }
      ],
      highlights: [
        {
  ...
  return [
    ...
    "All human-readable explanatory fields must be written in Simplified Chinese, including title, summary, simulation.summary, measurement descriptions, and highlight descriptions.",
    "Keep component labels exactly as they appear in the source image, for example R1, R2, S, A, V, P.",
    "",
    "Hard requirements:",
...
function extractOutputText(apiResponse, wireApi = DEFAULT_WIRE_API) {
  if (normalizeWireApi(wireApi) === "chat_completions") {
    const parts = [];
...
function normalizeUsage(apiResponse) {
...
function extractJsonObject(text) {
  ...
  for (const block of extractJsonCodeBlocks(normalized)) {
    const parsed = tryParseJsonCandidate(block);
    if (parsed) return parsed;
  }

  const candidates = extractBalancedJsonObjects(normalized).sort((a, b) => b.length - a.length);
  ...
  if (!candidates.length) {
    throw new Error(`No JSON object found in model response. Preview: ${excerpt}`);
  }
  throw new Error(`Failed to parse JSON object from model response. Preview: ${excerpt}`);
}

function isJsonExtractionError(error) {
  const message = String(error?.message || "");
  return message.includes("No JSON object found") || message.includes("Failed to parse JSON object");
}
...
function resolveParserOptions(meta) {
  ...
  return {
    wireApi: normalizeWireApi((typeof meta?.wireApi === "string" && meta.wireApi.trim()) || config.wireApi || DEFAULT_WIRE_API),
...
async function callModelApi(payload, wireApi) {
  const config = getEnvConfig();
  if (!config.apiKey) {
  ...
  function callPythonBridge(url, requestPayload) {
    const scriptPath = path.join(__dirname, "..", "scripts", "openai_http_bridge.py");
    ...
    return new Promise((resolve, reject) => {
      const child = execFile(
        ...
        (error, stdout, stderr) => {
          if (error) {
            const message = stderr?.trim() || error.message || "Python bridge failed";
            if (String(error.message || "").toLowerCase().includes("timed out")) {
              reject(buildUpstreamError("Upstream parse request timed out", true));
              return;
            }
            reject(buildUpstreamError(message, true));
            return;
          }
          ...
          try {
            ...
            const preview = String(stdout || stderr || "").slice(0, 300);
            reject(buildUpstreamError(`Python bridge returned invalid JSON: ${preview}`, false));
          }
        }
      );

      child.stdin.on("error", (stdinError) => reject(buildUpstreamError(stdinError.message || "Failed to send request to Python bridge", true)));
  ...
  let lastError = null;
  const normalizedWireApi = normalizeWireApi(wireApi || config.wireApi || DEFAULT_WIRE_API);
  const apiPath = normalizedWireApi === "chat_completions" ? "/chat/completions" : "/responses";
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt += 1) {
    ...
    try {
      if (normalizedWireApi === "chat_completions") {
        const result = await callPythonBridge(`${config.baseUrl}${apiPath}`, payload);
        const parsed = result?.body;

        if (!result?.ok) {
          const message = parsed?.error?.message || result?.error || `Upstream parse request failed (${result?.status || "network"})`;
          const retryable = isRetryableStatus(result?.status) || !result?.status;
          throw buildUpstreamError(message, retryable);
        }

        if (!parsed || typeof parsed !== "object") {
          throw buildUpstreamError("Upstream did not return JSON", false);
        }
...
async function requestSceneJson(meta, prompt, options) {
  const executeRequest = async (currentPrompt) => {
    if (options.wireApi === "chat_completions") {
      return callModelApi({
        ...
        messages: [
          {
            role: "system",
            content: "You are a precise multimodal parser for physics circuit diagrams. Return valid JSON only. All explanatory text fields must be Simplified Chinese."
          },
          {
            role: "user",
            content: [
              ...
              },
              {
                type: "image_url",
                image_url: {
                  url: meta.imageDataUrl,
                  detail: options.imageDetail || undefined
                }
              }
            ]
          }
        ],
        max_tokens: options.fastMode ? 3000 : 6000
      }, options.wireApi);
    }
    ...
    return callModelApi({
      ...
      instructions: "You are a precise multimodal parser for physics circuit diagrams. Return valid JSON only. All explanatory text fields must be Simplified Chinese.",
      text: {
  ...
  const primaryResponse = await executeRequest(prompt);
  const primaryText = extractOutputText(primaryResponse, options.wireApi);

  try {
    return {
      scene: extractJsonObject(primaryText),
      usage: normalizeUsage(primaryResponse),
      rawText: primaryText
    };
  } catch (error) {
    if (!isJsonExtractionError(error)) throw error;

    const retryResponse = await executeRequest(buildStrictJsonPrompt(prompt));
    const retryText = extractOutputText(retryResponse, options.wireApi);
    return {
      scene: extractJsonObject(retryText),
      usage: mergeUsage(normalizeUsage(primaryResponse), normalizeUsage(retryResponse)),
      rawText: retryText
    };
  }
}
...
function needsChineseFallback(text) {
  const value = String(text || "").trim();
  if (!value) return true;
  return !containsChinese(value) && /[A-Za-z]/.test(value);
}

function chineseComponentName(type) {
  const labels = {
    battery: "电源",
    switch: "开关",
    resistor: "电阻",
  ...
  return labels[type] || type || "元件";
}

function summarizeSceneTypesInChinese(scene) {
  const counts = new Map();
  ensureArray(scene?.components).forEach((component) => {
    const key = chineseComponentName(component?.type);
...
function buildChineseTitle(scene) {
  const types = ensureArray(scene?.components).map((component) => component?.type);
  if (types.includes("variable_resistor") && types.includes("voltmeter") && types.includes("ammeter")) {
    return "含滑动变阻器、电压表和电流表的电路图";
  }
  if (types.includes("lamp") && types.includes("switch")) {
    return "含开关和灯泡的电路图";
  }
  if (types.includes("resistor") && types.includes("switch") && types.includes("battery")) {
    return "含电阻、开关和电源的电路图";
  }
  const summary = summarizeSceneTypesInChinese(scene);
  return summary ? `由${summary}组成的电路图` : "电路图";
}

function buildChineseSummary(scene) {
  const summary = summarizeSceneTypesInChinese(scene);
  const interactive = ensureArray(scene?.simulation?.adjustables)
    .map((item) => item?.label)
    .filter(Boolean)
    .join("、");
  if (summary && interactive) {
    return `图中包含${summary}，可交互元件有${interactive}。`;
  }
  if (summary) {
    return `图中包含${summary}。`;
  }
...
function localizeSceneText(scene) {
  if (!scene || typeof scene !== "object") return scene;

  if (needsChineseFallback(scene.title)) {
    scene.title = buildChineseTitle(scene);
  }

  if (needsChineseFallback(scene.summary)) {
    scene.summary = buildChineseSummary(scene);
  }

  scene.simulation = scene.simulation || {};
  if (needsChineseFallback(scene.simulation.summary)) {
    scene.simulation.summary = scene.summary || buildChineseSummary(scene);
  }

  ensureArray(scene.simulation.adjustables).forEach((item) => {
    if (!needsChineseFallback(item?.label)) return;
    const component = ensureArray(scene.components).find((entry) => entry.id === item?.component_id);
    if (component?.type === "switch") {
      item.label = component?.label ? `开关 ${component.label}` : "开关";
      return;
    }
    if (component?.type === "variable_resistor") {
      item.label = component?.label ? `${component.label} 滑片` : "滑动变阻器滑片";
    }
  });

  ensureArray(scene.simulation.measurements).forEach((item, index) => {
    if (needsChineseFallback(item?.label)) {
      item.label = `测量值${index + 1}`;
    }
    if (needsChineseFallback(item?.description)) {
      item.description = `由电路参数推导出的测量值${index + 1}`;
    }
  });
...
async function parseCircuitImage(meta) {
  ...
  const finalScene = localizeSceneText(normalizeResolvedScene(normalizeSceneDocument(sourceScene, meta)));
  return {
    scene: finalScene,
