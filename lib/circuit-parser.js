...
async function requestSceneJson(meta, prompt, options) {
  const executeRequest = async (currentPrompt) => callResponsesApi({
    model: options.model,
    store: false,
  ...
  const primaryText = extractOutputText(primaryResponse);

  try {
    return {
      scene: extractJsonObject(primaryText),
      usage: primaryResponse.usage || null,
      rawText: primaryText
    };
  } catch (error) {
    ...
    return {
      ...
      usage: mergeUsage(primaryResponse.usage || null, retryResponse.usage || null),
      rawText: retryText
    };
  }
}
